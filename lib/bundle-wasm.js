...
let wabt;
async function bundleWasm(wabt_, watPath, wrap) {
  wabt = wabt_;
  ...
  let entryPointAst = absPath.endsWith('.wat')
    ...
    : readWasm(absPath);
  if (wrap) {
    addWrappers(entryPointAst, wrapperModules);
  }
  enumerateModules(absPath, moduleList, specialModules, entryPointAst);
...
function addWrappers(ast, wrapperModules) {
...
function enumerateModules(absPath, moduleList, specialModules = [], ast) {
    ...
    ModuleImport({node}) {
      ...
      if (isFile) {
        ...
        if (!moduleList.some(m => m.absPath === importAbsPath)) {
          enumerateModules(importAbsPath, moduleList, specialModules);
        }
      } else {
