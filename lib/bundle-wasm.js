...
async function bundleWasm(wabt_, watPath) {
  ...
  let moduleList = [];
  enumerateModules(memoryPath, moduleList, specialModules);
  let memoryImports = moduleList.find(m => m.absPath === memoryPath).imports;
  memoryImports.add('memory_alloc');
  memoryImports.add('memory_free');
  memoryImports.add('memory_memory');
