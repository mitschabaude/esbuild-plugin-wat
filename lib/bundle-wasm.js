...
import {global, module, program} from '@webassemblyjs/ast/lib/nodes';
import {instruction} from '@webassemblyjs/ast/lib/node-helpers';
...
async function bundleWasm(wabt_, watPath) {
  ...
  let globalDataEnd = global('i32', [instruction(identifier())]);
  let metaModuleAst = program([module(), []]);
  ...
  let moduleList = [
    {
      ...
      baseName: 'common',
      name: 'common',
      ast: readWat(commonPath),
      imports: new Set(['common_alloc', 'common_free', 'common_memory']),
      dataEnd: 0,
    },
  ];
  moduleList = enumerateModules(absPath, moduleList);
  console.log(moduleList);

  let entryPoint = moduleList[moduleList.length - 1];
  entryPoint.imports.add('*');

  // console.log(entryPoint.ast.body[0].metadata);
  ...
  let fixedImports = [
    ...
    moduleImport('./common', 'common_memory', {
      type: 'Memory',
