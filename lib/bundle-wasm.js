...
function enumerateModules(
  ...
  traverse(ast, {
    ModuleImport({node}) {
      ...
      // try to resolve
      let isWasmFile = false;
      let importAbsPath;
      try {
        if (!special) {
          importAbsPath = require.resolve(node.module);
          isWasmFile = isWasm.test(importAbsPath);
        }
      } catch {}

      if (
        untouchedImports.has(node.module) ||
        (!(importAbsPath && isWasmFile) && !special)
      ) {
        otherImports[node.module] ??= [];
        let imports = otherImports[node.module];
        if (!imports.includes(node.name)) imports.push(node.name);
        return;
      }
      ...
      node.module = 'wasm-file://' + mod.name;
  ...
  for (let node of fields) {
    switch (node.type) {
      ...
      case 'ModuleImport': {
        ...
        let isBundled = node.module.startsWith('wasm-file://');
        if (!isBundled) {
          keep.add(node);
        }
        let {descr, name} = node;
        if (isBundled && !importMap[name]) {
          let modName = node.module.slice(12);
